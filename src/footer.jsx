    ...
    <footer className={`${isDarkMode ? 'bg-black border-gray-700 text-gray-300' : 'bg-white border-gray-200 text-gray-700'} border-t pt-10 pb-4 px-4 md:px-0`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-5 gap-8 pb-8" dir={isRTL ? 'rtl' : 'ltr'}>
         ...
         <div className={`flex flex-col space-y-3 ${isRTL ? 'items-end' : 'items-start'}`} dir={isRTL ? 'rtl' : 'ltr'}>
           ...
           <span className={`font-bold text-lg text-[#FF4D00] ${isRTL ? 'text-right' : 'text-left'}`}>{translate('buildRightConstruction')}</span>
           <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} ${isRTL ? 'text-right' : 'text-left'}`}>{translate('footerTagline')}</span>
         </div>
        {/* Quick Navigation */}
        <div dir={isRTL ? 'rtl' : 'ltr'}>
          <h3 className={`font-semibold mb-3 text-[#FF4D00] ${isRTL ? 'text-right' : 'text-left'}`}>{translate('quickLinks')}</h3>
          <ul className="space-y-2">
            ...
            <li><Link to="/services" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('services')}</Link></li>
                 ...
                 {/* Explore Services */}
         <div dir={isRTL ? 'rtl' : 'ltr'}>
           <h3 className={`font-semibold mb-3 text-[#FF4D00] ${isRTL ? 'text-right' : 'text-left'}`}>{translate('services')}</h3>
           <ul className="space-y-2">
             <li><Link to="/yoga" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('residentialConstruction')}</Link></li>
             <li><Link to="/commercial" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('commercialConstruction')}</Link></li>
             <li><Link to="/renovation" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('renovationRemodeling')}</Link></li>
             <li><Link to="/interior-design" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('interiorDesign')}</Link></li>
             <li><Link to="/industrial" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('industrialConstruction')}</Link></li>
             <li><Link to="/road" className={`hover:text-blue-600 ${isRTL ? 'text-right' : 'text-left'} block`}>{translate('road')}</Link></li>
           </ul>
         </div>
                 {/* Contact Info */}
         <div dir={isRTL ? 'rtl' : 'ltr'}>
           <h3 className={`font-semibold mb-3 text-[#FF4D00] ${isRTL ? 'text-right' : 'text-left'}`}>{translate('contactUs')}</h3>
           <ul className={`space-y-2 text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
             <li><span className="font-medium">{translate('address')}:</span> {translate('footerAddress')}</li>
             <li><span className="font-medium">{translate('phone')}:</span> (555) 987-6543</li>
             <li><span className="font-medium">{translate('email')}:</span> <a href="mailto:contact@buildright.com" className="hover:text-blue-600">contact@buildright.com</a></li>
             <li><span className="font-medium">{translate('hours')}:</span> {translate('footerHours')}</li>
           </ul>
           {/* Social Media Icons */}
           <div className="mt-4">
             <h4 className={`font-medium mb-2 text-sm text-[#FF4D00] ${isRTL ? 'text-right' : 'text-left'}`}>{translate('followUs')}</h4>
        ...
        {/* Newsletter Signup */}
        <div dir={isRTL ? 'rtl' : 'ltr'}>
          <h3 className={`font-semibold mb-3 text-[#FF4D00] ${isRTL ? 'text-right' : 'text-left'}`}>{translate('newsletter')}</h3>
          ...
          <form className={`flex flex-col sm:flex-row gap-2 w-full ${isRTL ? 'sm:flex-row-reverse' : ''}`} onSubmit={handleSubscribe}>
            <input
              ...
              className={`px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#FF4D00] flex-1 min-w-0 ${isRTL ? 'text-right' : 'text-left'}`}
              value={email}
          ...
          {subscribed && (
            <div className={`mt-2 text-[#FF4D00] text-sm font-semibold ${isRTL ? 'text-right' : 'text-left'}`}>{translate('subscriptionThankYou')}</div>
          )}
        </div>
      </div>
             {/* Legal Links */}
